...
<template>
                ...
                <div>
                    ...
                    <p class="text-slate-500 mt-1 text-sm">
                        ...
                        <span class="font-semibold text-slate-700">{{
                            props.departemen || "Biro Perencanaan"
                        }}</span>
                    </p>
                </div>

                <div
                    class="flex items-center gap-3 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200"
                >
                    <button
                        type="button"
            ...
            >
                ...
                <div class="grid grid-cols-7 auto-rows-fr bg-slate-200 gap-px">
                    ...
                    >
                        ...
                        <div
                            class="space-y-1 mt-1 overflow-y-auto max-h-[60px]"
                            v-if="day.isCurrentMonth"
                        >
                            <template
                                v-for="cuti in getCutiOnDate(day.dateString)"
                                :key="cuti.id"
                            >
                                <div
    ...
    <Teleport to="body">
        ...
        >
            <div
                class="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in duration-200"
            >
                <div
                    class="bg-slate-900 text-white px-6 py-4 flex justify-between items-center"
                >
                    <div>
                        <h3 class="font-bold text-base">Informasi Jadwal</h3>
                        <p class="text-xs text-slate-400 mt-0.5">
                            Tanggal: {{ selectedDateData.date }}
                        </p>
                    </div>
                    <button
                        type="button"
                        @click.prevent="closeModal"
                        class="text-slate-400 hover:text-white bg-slate-800 p-1.5 rounded-full cursor-pointer transition"
                    >
                        <svg
                            class="w-5 h-5"
                            fill="none"
                ...
                <div class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div
                        ...
                        class="p-4 rounded-xl bg-red-50 border border-red-200 text-red-700 flex items-start gap-3"
                    >
                        <svg
                    ...
                    <div v-if="selectedDateData.list.length > 0">
                        ...
                        <div class="space-y-3">
                            <div
                                v-for="item in selectedDateData.list"
                                :key="item.id"
                            ...
                            >
                                <div class="flex justify-between items-start">
                                    <div>
                                        <span
                                            class="font-bold text-slate-800 text-sm block"
                                            >{{ item.pegawai?.nama }}</span
                                        >
                                        <span
                                            class="text-[10px] font-semibold bg-green-100 text-green-700 px-2 py-0.5 rounded-full mt-1 inline-block"
                                            >Disetujui</span
                                        >
                                    </div>
                                </div>
                                ...
                                <div
                                    class="text-xs text-slate-600 space-y-1 bg-white p-2.5 rounded-lg border border-slate-100"
                                >
                                    <p>
                                        <span
                                            class="font-medium text-slate-400 block mb-0.5"
                                            >Durasi Cuti:</span
                                        >
                ...
                <div
                    class="bg-slate-50 px-6 py-3 border-t border-slate-100 flex justify-end"
                >
                    <button
                        type="button"
                        @click.prevent="closeModal"
                        class="px-5 py-2 bg-slate-800 text-white rounded-xl text-sm font-semibold hover:bg-slate-900 transition cursor-pointer shadow-sm"
                    >
                        Tutup
                    </button>
                </div>
